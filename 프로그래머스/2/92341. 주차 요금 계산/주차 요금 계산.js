...
const solution = (fees, records) => {
    ...
    const sub = {};
    const another = new Set();
    const cal = (standardPrice, accumulated, standard, unit, unitPrice) => {
    ...
    records.forEach((record) => {
        ...
        sub[num][io]++;
        if(io === "IN"){
            recording[num] = (recording[num] || 0) - intoM;
        }else{
            recording[num] += intoM;
        }
        another.add(num);
    ...
    another2.forEach((number) => {
        if(sub[number]["IN"] !== sub[number]["OUT"]) recording[number] += condition;
        const acc = recording[number]
        const fee = cal(standardPrice, acc, standard, unit, unitPrice);
        result.push(fee);
    })
